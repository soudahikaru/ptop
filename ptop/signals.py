...
from .models import SupplyItem, TroubleGroup
...
def trouble_group_pre_save_receiver(sender, instance, *args, **kwargs):
    if not instance.path:
        ...
        if not instance.path.endswith(finalpath):
            ...
            q = TroubleGroup.objects.filter(
                path__exact=parent_path).exclude(id__exact=instance.id)
            if q is None:
                print(
                    'signal.py: on create_group, path exists but search result of path except itself is None')
                return
            parent = q[0]
...
def supply_item_post_save_receiver(sender, instance, *args, **kwargs):
    if (instance.stock_date is not None) and (instance.install_date is None) and (instance.is_available is False):
